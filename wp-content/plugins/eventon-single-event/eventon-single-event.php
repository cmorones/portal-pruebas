...
class EventON_sin_event{
	...
	/* Construct */
		public function __construct(){		
			...
			if($check){
				...
				add_action( 'eventon_enqueue_scripts', array( $this, 'eventon_scripts' ) );	
	...
	// enqueue single event scripts to eventon pages
		function eventon_scripts(){
			wp_register_script('evoSE_script', $this->plugin_url. '/assets/se_evo_script.js', array('jquery'),'1.0',true );
	...
	// facebook header to single events pages
		function fb_headers(){
			...
			if($post && $post->post_type=='ajde_events'):
				...
				<meta property="description" content="<?php echo $this->get_excerpt(25, $post->post_content);?>" />
				<meta property="og:url" content="<?php echo get_permalink($post->ID);?>" /> 
				<meta property="og:title" content="<?php echo $post->post_title;?>" />
				<meta property="og:description" content="<?php echo $this->get_excerpt(25, $post->post_content);?>" />
				<?php if($img_id!=''): 
		...
		function get_excerpt($limit, $content){
			global $post;
			...
			if( (!empty($evo_opt['evosm_som']) && $evo_opt['evosm_som']=='yes' && $__calendar_type=='single') || ( empty($evo_opt['evosm_som']) ) || ( !empty($evo_opt['evosm_som']) && $evo_opt['evosm_som']=='no' ) ){
				...
				$social_sites = apply_filters('evo_se_social_media', array(
					...
					'Google' => Array (
						...
						'url' => '<a class="gp evo_ss" href="https://plus.google.com/share?url=PERMALINKCOUNT" 
