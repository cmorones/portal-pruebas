...
jQuery(document).ready(function($){
	...
	// AJAX:  when changing date
		function ajax_update_month_events(cal_id, new_day){
			...
			var new_date_el = ev_cal.find('#evcal_head .evoDV_val');
			var new_day_ =1;
			...
			$.ajax({
				beforeSend: function(){
					ev_cal.find('.eventon_events_list').slideUp('fast');
                      /* ispirazione */ ev_cal.find('#fakeLoader').remove();
				...
				success:function(data){
					...
					ev_cal.find('.eventon_other_vals').val(new_day_);
                                         ...
                                         /* ispirazione */  
                    $( ".eventosslider" ).find('.evoDV').append('<div id="fakeLoader"></div>');
                    ev_cal.find('#evcal_list').append('<div class="bxCerrarEvnt" title="Ocultar listado"></div>');
                ...
                ev_cal.find('#fakeLoader').fakeLoader({
                    timeToHide: 1200,
                    zIndex: "999",
                    spinner: "spinner6", 
