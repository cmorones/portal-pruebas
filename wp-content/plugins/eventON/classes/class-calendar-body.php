		...
		public function cal_above_header($args){
			...
			if($jumper=='yes'){
				...
				$jumper_content.= "<div class='evo_j_container' style='display:none' data-m='{$focused_month_num}' data-y='{$focused_year}'>
					...
					// months list
					$__months = eventon_get_oneL_months($this->cal->evopt2['L1']);				
					$count = 1;
		...
		function get_calendar_header($arguments){
			...
			// calendar data variables
				...
				$cdata = apply_filters('eventon_cal_jqdata', array(
					...
					'mapformat'=> (($this->cal->evopt1['evcal_gmap_format']!='')?$this->cal->evopt1['evcal_gmap_format']:'roadmap'),
					'mapzoom'=>(($this->cal->evopt1['evcal_gmap_zoomlevel']!='')?$this->cal->evopt1['evcal_gmap_zoomlevel']:'12'),
