...
// ==========================
//	TAXONOMY
	add_action( 'admin_init', 'eventon_taxonomy_admin' );
	function eventon_taxonomy_admin(){
		global $pagenow;
		if($pagenow =='edit-tags.php' && !empty($_GET['taxonomy']) && $_GET['taxonomy']=='event_location' && !empty($_GET['post_type']) && $_GET['post_type']=='ajde_events'){
			wp_enqueue_media();
		}
	}
...
// add term page
	function eventon_taxonomy_add_new_meta_field() {
		...
		<div class="form-field evo_metafield_image">
			...
			<span class='evo_loc_image_src'><img src='' style='display:none'/></span>
			
			<input class='evo_loc_img' type="hidden" name="term_meta[evo_loc_img]" id="term_meta[evo_loc_img]" value="">
	...
	function eventon_taxonomy_edit_meta_field($term) {
		...
		<tr class="form-field">
			...
			<td class='evo_metafield_image'>
				...
				<span class='evo_loc_image_src'><img src='<?php echo $img_url[0];?>' style='<?php echo !empty($term_meta['evo_loc_img'])?'':'display:none';?>'/></span>
				
				<input class='evo_loc_img' type="hidden" name="term_meta[evo_loc_img]" id="term_meta[evo_loc_img]" value="<?php echo !empty( $term_meta['evo_loc_img'] ) ? esc_attr( $term_meta['evo_loc_img'] ) : ''; ?>">
				<p class="description"><?php _e( '(Optional) Location Image','eventon' ); ?></p>
			</td>
		</tr>
	<?php
...
// Event Organizer
	// remove some columns
		add_filter("manage_edit-event_organizer_columns", 'eventon_evorganizer_theme_columns');  
	...
	// Add event organizer address field  
		add_filter("manage_event_organizer_custom_column", 'eventon_manage_evorganizer_columns', 10, 3); 
	...
	// add term page
		function eventon_taxonomy_add_new_meta_field_org() {
			...
			<?php /*
			<div class="form-field">
				<label for="term_meta[evcal_org_img]"><?php _e( 'Image URL', 'eventon' ); ?></label>
				<input type="text" name="term_meta[evcal_org_img]" id="term_meta[evcal_org_img]" value="">
				<p class="description"><?php _e( 'Organizer Image URL','eventon' ); ?></p>
			</div>
			*/?>
		<?php
		}
		add_action( 'event_organizer_add_form_fields', 'eventon_taxonomy_add_new_meta_field_org', 10, 2 );
	// Edit term
		function eventon_taxonomy_edit_meta_field_org($term) {
			...
			<?php /*
			<tr class="form-field">
			<th scope="row" valign="top"><label for="term_meta[evcal_org_img]"><?php _e( 'Image URL', 'eventon' ); ?></label></th>
				<td>
					<input type="text" name="term_meta[evcal_org_img]" id="term_meta[evcal_org_img]" value="<?php echo esc_attr( $term_meta['evcal_org_img'] ) ? esc_attr( $term_meta['evcal_org_img'] ) : ''; ?>">
					<p class="description"><?php _e( 'Organizer Image URL','eventon' ); ?></p>
				</td>
			</tr>
			*/?>
		...
		add_action( 'event_organizer_edit_form_fields', 'eventon_taxonomy_edit_meta_field_org', 10, 2 );

	// save terms
		function evo_save_taxonomy_custom_meta_org( $term_id ) {
			if ( isset( $_POST['term_meta'] ) ) {
				$t_id = $term_id;
				$term_meta = get_option( "taxonomy_$t_id" );
				$cat_keys = array_keys( $_POST['term_meta'] );
		...
		add_action( 'edited_event_organizer', 'evo_save_taxonomy_custom_meta_org', 10, 2 );  
		add_action( 'create_event_organizer', 'evo_save_taxonomy_custom_meta_org', 10, 2 );
?>