...
class evo_ajax{
	...
	public function __construct(){
		$ajax_events = array(
			...
			'verify_lic'=>'eventon_license_verification',
			'verify_lic_2'=>'save_envatoVerified_license',
			'deactivate_lic'=>'eventon_deactivate_evo',
			'addon_lic_activate'=>'eventon_addon_license_activation',
	...
	// Primary function to load event data 
		function evcal_ajax_callback(){
			...
			// generate new UNIX range dates for calendar
				if($send_unix=='1'){
					$focus_start_date_range = (isset($evodata['focus_start_date_range']))? (int)($evodata['focus_start_date_range']):null;
					$focus_end_date_range = (isset($evodata['focus_end_date_range']))? (int)($evodata['focus_end_date_range']):null;	
	...
	// Verify EVENTON Licenses AJAX function
		function eventon_license_verification(){
			...
			// verify purchase code format is correct
				$verifyformat = $eventon->evo_updater->purchase_key_format($key);
				if(!$verifyformat){
					$status='error';	
					$error_msg = $eventon->evo_updater->error_code_('10');
					$error_code = '10';
					$debug.='x';
				}


				// proceed to next steps if code format is correct
				if($status!='error'){
					// verify license from eventon server
					$return = $eventon->evo_updater->_verify_license_key($slug, $key);

					// json api link passed
					if(strpos($return, 'http')!== false){
						$new_license_content = $return;
						$status ='json';
						$error_msg = $eventon->evo_updater->error_code_( $eventon->evo_updater->error_code );
						$error_code = $eventon->evo_updater->error_code;
						$debug.=1;
					}elseif($return=='1'){
	...
	// save locally verified license
		function save_envatoVerified_license(){
			global $eventon;	
			$new_license_content= '';
			...
			$save_license_date = $eventon->evo_updater->save_license_key($slug, $key, 'locally');
									
			// successfully saved new verified license
			if($save_license_date!=false){
				$status = 'success';				
				$new_license_content ="License Status: <strong>Activated</strong>";
			}else{// could not save licenses
				$status='error';				
				$error_msg = 'Could not save verifies license to system.';
			}
			$return_content = array(
				'status'=>$status,		
				'new_content'=>$new_license_content,
